            ...
            <b-steps v-if="visivel" v-model="activeStep" :animated="isAnimated" :rounded="isRounded"
              ...
              <b-step-item :step="stepCobranca" label="Cobrança" :clickable="isStepsClickable">
                <Cobranca @change="updateValue" :telefone="this.dadosForm['celular']"
                  :valorSubtotalMensalidade="valorSubtotalMensalidade" :valorSubtotalExtra="valorSubtotalExtra"
                  :valorSubtotalCremacao="valorSubtotalCremacao" />
              </b-step-item>
...
export default {
  ...
  computed: {
    valorSubtotalMensalidade() {
      ...
      for (let i = 0; i < this.$store.state.mensalidades.length; i++) {
        if (this.idade >= this.$store.state.mensalidades[i].IDADE_MIN && this.idade <= this.$store.state.mensalidades[i].IDADE_MAX)
          val = this.$store.state.mensalidades[i].VALOR;
      }
      return val == 0 ? 0 : Number(val.replace(",", "."));
    },
    valorSubtotalCremacao() {
      let val = 0;
      if (this.$store.state.proposta.crematorio == 's')
        val = this.$store.state.cremacao;

      return val;
    },
    valorSubtotalExtra() {
      let val = 0;
      for (let index = 0; index < this.$store.state.proposta.dependentes.length; index++) {
        if (this.$store.state.proposta.dependentes[index])
          val += this.$store.state.proposta.dependentes[index].valor;
      }
      return val;
    },
    ...
    idade() {
      ...
      let d = this.$store.state.proposta.dataNasc.split('-');
    ...
    canGoNext() {
      ...
      switch (this.activeStep) {
        ...
        case 1:
          ...
          if (typeof this.dadosForm["sexo"] === 'undefined' || this.dadosForm["sexo"] === null) {
            res = false;
          }
          ...
          for (let index = 0; index < this.$store.state.proposta.dependentes.length && c; index++) {
            ...
            c = (d.nome.length != 0 && cpf.isValid(d.cpf) && d.dataNasc != "" && d.grau != -1 && d.sexo != '');
          }
          if (!c) {
            res = false;
          }
            ...
            if (Array.isArray(this.$store.state.proposta.beneficiarios)) {
              for (let i = 0; i < this.$store.state.proposta.beneficiarios.length; i++) {
                res = (this.isValid(this.$store.state.proposta.beneficiarios[i].nome) &&
                  ...
                  this.isValid(this.$store.state.proposta.beneficiarios[i].dataNasc) &&
                  this.isValid(this.$store.state.proposta.beneficiarios[i].grau) &&
